...
void FB2_ElkanKmeans::update_center_dists(int threadId) {
...
int FB2_ElkanKmeans::runThread(int threadId, int maxIterations) {
    int iterations = 0;
    ...
    while ((iterations < maxIterations) && ! converged) {
        ...
        if (iterations > 1){
            for (int c1 = 0; c1 < k; ++c1) {
                for (int i = 0; i < 1+oldclusterp[c1].size;++i){
                    clusterp[c1][i];
                }
            }
        }
                ...
                if (clusterassignment[c1].size()!=0){
                    for (int i = 0; i < oldclusterp[c1].size();++i){
                        ...
                        for (int j = 0; j < clusterassignment[c1].size(); ++j) {
                            ...
                            //if (upper[i] <= lower3[i * k + j]) { continue; }
                            if (upper[oldclusterp[c1][i]] <= lower[oldclusterp[c1][i] * k + clusterassignment[c1][j]]) { continue; }
                            if (upper[oldclusterp[c1][i]] <= centerCenterDistDiv2[closest * k + clusterassignment[c1][j]]) { continue; }
                            if (upper[oldclusterp[c1][i]] <= oldcenter2newcenterDis[assignment[oldclusterp[c1][i]] * k + clusterassignment[c1][j]] - ub_old[oldclusterp[c1][i]]) { continue; }
                            #if Countdistance
                            numberdistances ++;
                            #endif
                            // ELKAN 3(a)
...
void FB2_ElkanKmeans::update_bounds(int startNdx, int endNdx) {
    //lower3
    /*
    for (int i = startNdx; i < endNdx; ++i) {
...
void FB2_ElkanKmeans::initialize(Dataset const *aX, unsigned short aK, unsigned short *initialAssignment, int aNumThreads) {
    numLowerBounds = aK;
...
int FB2_ElkanKmeans::move_centers_newbound(double *oldcenters, double *oldcenter2newcenterDis) {
